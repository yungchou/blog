...
if(start.from.scratch){
  ...
  # Data Preparation
  #------------------
  source('yc.housing.rtm.data.prep.R')   # Prepare train.prep
  train.prep <- read.csv('data/train.org.prep.csv')
...
boruta.selected.features <- readRDS('boruta/train.boruta.selected.features.rds')
...
#----------------------------
#names(train.imp)
train <- train.imp[c(boruta.selected.features,'SalePrice')]
...
train <- train[-c(314,1025,524,1299,336
                  ,1325
                  ,584,1374
                  ,707,225
                  ,770,534
...
removeThese <- TRUE  # It turned out removing these, reduce R^2
if(removeThese){

  #summary(train$GrLivArea)
  #boxplot(train$GrLivArea, main='GrLivArea')
  #sum(train$GrLivArea>40000)
  train <- train[train$GrLivArea<4000,]  # remove this actually reduced R^2

  #summary(train$TotalBsmtSF)
  #boxplot(train$TotalBsmtSF, main='TotalBsmtSF')
  #sum(train$TotalBsmtSF>3000)
  train <- train[train$TotalBsmtSF<3000,]  # remove this actually reduced R^2

  #summary(train$BedroomAbvGr)
  #boxplot(train$BedroomAbvGr, main='BedroomAbvGr')
  #sum(train$BedroomAbvGr>8)
  train <- train[train$BedroomAbvGr<8,]  # remove this actually reduced R^2
...
#-------------------
set.seed(0-0)
part <- sample(2,nrow(train),replace=TRUE,prob=c(0.5,0.5))
train.set <- train[part==1,]
test.set  <- train[part==2,]

# Make train and test with the same number of rows
n <- min(nrow(train.set), nrow(test.set))
train.set <- train.set[1:n,]
test.set <- test.set[1:n,]
...
# 3. Lasso Regression
#---------------------
grid = seq(0.001, 10, length = 10)

set.seed(3-3)
lasso <- train(SalePrice~(OverallQual
                          #+GrLivArea
)*(
  BedroomAbvGr
  +X1stFlrSF
...
test.prep <- read.csv('data/test.org.prep.csv')

test <- test.prep[c(boruta.selected.features)]
...
write.csv(prediction.lasso, 'results/prediction.house.price.3.lasso.csv' )
